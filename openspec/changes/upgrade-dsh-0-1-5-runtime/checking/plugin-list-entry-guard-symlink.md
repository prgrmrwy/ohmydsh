...
## 端到端验证:经 `bin/dsh` 的真实路径(已补做)

上表证明的是"脚本本身修好了"。真正要证明的是**`bin/dsh` 消费它时能拿到清单**。
按 9.1 的做法推一个高熵验收 ref,让 devbox 取到最终候选并重启 Host:

| 步骤 | 观测 | 结果 |
| --- | --- | --- |
| 候选身份 | expected = fetched = HEAD | 三者相等 ✅ |
| checkout | 工作区改动 | 0,入口守卫已含(`isEntryPoint` 命中 1) |
| **重启前** | 脚本直接跑 `--names` | **22** 条目(已修) |
| **重启前** | 启动日志最后一条的 `plugins` 条目数 | **0** ← 修复前的历史残留 |
| 重启 | `dsh restart`(重启前 shell 代理变量数 **0**) | exit **0**,Host pid `2308857` → **`2376226`**,`dshVersion=0.1.5-rc.2` |
| **重启后** | 启动日志**新增**一条 `[2026-09-22T03:37:59+0800]` | **`plugins` = 22 条** ✅ |
| 重启后 | 其中含 `client-connection` | **1** ✅ |
| 重启后 | Host 进程代理变量数 | **0** ✅ |

即:**44 条 `plugins=[]` 的历史到此终止**,启动 msg 与启动日志同时恢复。
两个缺陷(覆盖式接线隐身 + 入口守卫失效)都已在真实消费路径上消除。

## 仍未覆盖

- 其它仓库脚本未做同类审计的**运行期**验证;本次只做了静态扫描(结果:仅此一处命中)。
- Windows 与非 POSIX 路径形态未测。
- 未验证"路径含需要百分号转义的字符(空格/非 ASCII)"这一情形 —— 修复用的
  `pathToFileURL` 本身会正确处理转义,故属于顺带修好而非专门验证。
