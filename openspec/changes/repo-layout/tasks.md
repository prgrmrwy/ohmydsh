...
- [ ] 2.2 编写根级 `dsh.yaml`(DSH 版本 0.1.0-rc.6,customizations 初始为空列表)
- [ ] 2.3 编写仓库根 `README.md`:说明真相源约定、sync 用法、禁用≠删除、手改回写规则

## 3. sync 工具

- [ ] 3.1 实现 `scripts/sync.mjs`:读取 manifest,按 type 分发物化(package → `dsh plugin add`;preset → 链接/复制;patch → 合并生成;skill → 落点方案)
...
- [ ] 4.3 按 spec 场景验收:manifest 缺失报错、disabled 不物化、toggle 可逆、双 patch 合并顺序、独立版本
