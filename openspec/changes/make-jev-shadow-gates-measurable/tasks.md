...
- [ ] 1.1 Add failing tests for schema-v2 bounded `language`, `sampleProvenance`, and explicit measured/unavailable latency normalization, including rejection/defaulting of arbitrary strings and non-finite values
...
- [ ] 2.1 Add failing report tests that separate `real-vibe`, `synthetic-fixture`, and unknown provenance and count `zh`, `en`, `mixed`, and unknown language coverage
...
- [ ] 2.3 Implement overall and real-vibe-only aggregates plus per-language, provenance, route, high-cost category, router-version, and candidate-availability breakdowns
...
- [ ] 3.1 Update the router skill to define bounded local-only language/provenance fields and prohibit sending them or raw request text to Jev
...
- [ ] 3.3 Extend privacy allowlist tests to prove arbitrary language/provenance strings, prompts, paths, URLs, identifiers, attachments, provider errors, and credentials cannot persist or appear in aggregate output
