...
见 `proposal.md`。当前 schema v1 只记录有界任务特征、推荐、usage 和整数 `latencyMs`；`0` 同时承担“未测量”占位。报告固定把语言和 provenance 声明为 unavailable，却又把语言覆盖和真实样本识别列为 Phase 2 强制门槛。因此门禁虽保守，却不能随真实使用收敛。

约束：记录仍位于本机受管状态目录；不能读历史 prompt 回填；不能增加自由文本或把语言/provenance 发送给 Jev；Phase 1 必须继续零执行权威；旧记录不能因 schema 升级被丢弃或错误补值。
...
### D2. 语言与 provenance 是本地记录元数据，不是 Jev feature

`language` 为 `zh|en|mixed|unknown`，只根据当前请求是否包含中文/英文文字在调用点确定；不可判定时 unknown。`sampleProvenance` 为 `real-vibe|synthetic-fixture|unknown`，普通 Agent 真实决策点固定 real-vibe，测试 fixture 必须显式 synthetic-fixture。二者写入 recorder，但不进入 classify/decide context。

这样可满足覆盖统计，同时不向 provider 扩大请求内容。自由文本、locale、session id 和 prompt hash 均不记录。
...
总体诊断可包含所有 records，但 Phase 2 dataset/quality 分母只使用 `sampleProvenance=real-vibe` 且可靠标注的合格记录；synthetic 只证明实现，不贡献真实覆盖。语言、route、高代价类别、版本、candidate availability 都生成 breakdown。每个 gate 输出 `pass|fail|unavailable`，整体只有全部 pass 时才可成为 `eligible-for-user-approval`；由于本 change 不承载用户的 Phase 2 批准，运行时仍不改变模式。
...
- **[mixed/unknown 使语言门槛收敛较慢]** → 保守优于猜测；报告按类别显示缺口。
