...
- [x] 3.1 定义 dsh-memex **自己命名**的浏览地址注册点与契约（命名权归 dsh-memex），契约为**异步**解析（注册方需与外部往返），写进本仓 spec 作为唯一真相源
- [x] 3.2 在 client 半区 `ctx.provide` 该注册点（提供方能力只存在于浏览器，host 侧注册点接不上）；实现注册/注销与默认实现：未注册时给出 `localhost:<实际端口>`；注册点 MUST NOT 进入 `inject`（loader 对不可解析依赖是静默不加载）
- [x] 3.3 注册方晚于 dsh-memex 加载也生效；注册方卸载后恢复默认且不持有失效引用
- [x] 3.4 注册方抛错或未给出地址时**不回落本机地址**（跨机器下会解析到错误的机器），改为呈现失败原因
- [x] 3.5 代码与配置中不出现任何注册方的 package 名 / 服务名 / 产品名（含约定服务名）
- [x] 3.6 单测：无注册方给本机地址；有注册方用其地址；晚注册生效；卸载恢复默认；注册方抛错不给本机地址也不报成功
