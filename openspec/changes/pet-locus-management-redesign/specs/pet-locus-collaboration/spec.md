...
### Requirement: 管理面按父会话与入口状态筛选且不静默隐藏

管理面 SHALL 默认只呈现父会话可用、且当前代仍可被服务的关联（`active` / `provisioning` / `switching`）。被该默认筛选排除的关联 MUST NOT 静默消失：管理面 SHALL 在列表内提供可发现的出口，说明被隐藏的父会话与入口数量及各自原因（父会话可用性、入口生命周期状态），并允许一次性显示。筛选控件 SHALL 表达当前生效的条件（父会话维度与入口状态维度都要表达），MUST NOT 仅以一个无语义的图标表示。筛选 SHALL 只影响呈现，MUST NOT 改变任何持久化状态，也 MUST NOT 被表述为已修改数据。默认隐藏终止态时，管理面 SHALL 让这些入口的**恢复路径**（重建）在显示后位于该行本身，MUST NOT 要求先展开行内折叠区；无可见行时 SHALL 在空态文案里指出出口，而不是让入口读起来像已被删除。
...
#### Scenario: 默认隐藏不再被服务的入口并给出出口

- **WHEN** 某入口当前代为 `stopped`（Host 此后拒绝该 endpoint 的工作，直到显式重建）
- **THEN** 默认视图不把它呈现为活跃入口，但列表内显示「已隐藏 N 个入口（按状态：已停止 N）」，可一键显示；显示后该行可直接执行「重建」，无需再展开其它折叠区
