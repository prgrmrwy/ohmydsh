...
- [x] 3.2 把 safe composition 安装纳入 prepublication 原子边界与冷恢复核验；父/user preset 漂移、probe 缺失或安装结果不可证明时拒绝创建/恢复/派发，不静默继承父 preset。（2026-09-19 完成：漂移已消除——locus 主会话改为固定组合 `LOCUS_MAIN_PRESET = 'dsh-pet-executor'`，child 从 `composedPreset(parent.ctx)` 继承它，不再受 Host 默认或用户选择影响；并由 `test/loader-composition.test.ts` 的守卫用例钉住「executor preset 的委派行不得携带 `modelSelectionSettings`」（实测注入该开关即失败）。发布前证明由 **3.6** 完成并在 20:25 真机确认。**已知残留**：修复前创建的 child 在 descriptor 里带着 `agentPreset: standard`，冷恢复仍保留 `subagent`——本轮所有者决定不处理存量（那三个入口要么是测试群、要么是 `invalid` 不服务流量的旧代际）。）
- [x] 3.3 更新 child prompt，删除直接 `lark-cli` 读写与“回复由你自己发出”的指引，明确业务正文只由 `pet_locus_finish` 产生；确认 prompt 不是唯一防线。
- [x] 3.4 添加真实负向隔离测试，尝试 `lark-cli`、绝对路径脚本、`msg.py`、Python/Node/curl、复制可执行文件及子委派均不能产生飞书消息；同时验证受管 finish 仍能发送并落账。（2026-09-19 完成：「子委派」一项已在真机复验——修复后新 locus 的 child 工具面无 `subagent`，且 `pet_locus_finish` 仍正常发送并落账；并新增 `test/loader-composition.test.ts` 的 preset 守卫用例。own 层分层重写由 **3.7** 完成。）
- [x] 3.5 为 safe composition/runtime seam 添加启动 probe、明确运维诊断与 fail-closed tests；compat patch 回滚或版本不匹配时 Locus 必须 unavailable。
- [x] 3.6 发布前组合证明（3.2 的剩余部分）：在写 `safe-v1` 之前，用真实 `tools.schemas(childScope)` 读出该 child **实际可见**的工具名，断言 ⊆ `LOCUS_SAFE_TOOL_FILTER.allow` ∪ Pet 自己的 caller-bound 工具；不满足则拒绝发布该代际，冷恢复同样校验。只靠「固定 preset + 配置守卫用例」只能防住**已知的** own 层泄漏路径，无法发现上游注册方式变化、preset 被改动或新增工具带来的新泄漏——本轮真机逃逸正是这类。（2026-09-19 已实现：`attestLocusComposition` + `LOCUS_SAFE_TOOL_NAMES`/`LOCUS_CALLER_BOUND_TOOLS` 权威清单 + `LocusScopedSurfacePort.visibleTools` 端口，在 `composeLocusChild` 里于 `surface.install()` 之后读取并拒绝，新增拒绝码 `surface-not-attested`；不可读与读取抛错都 fail closed。证明落在 agent 创建边界上，所以新建与冷恢复同一条路径都过它。生产读取的可用性**由构造保证**：`agents` 与 `tools` 都在插件 `inject` 列表里，而 Cordis 的 `inject` 是加载前置条件，因此接线用惯用的声明式访问（`ctx.agents.get` / `ctx.tools.schemas`），不存在「服务缺失导致读不到」的退化路径。**真机确认（20:25）**：Host 重启（PID `70182`）后冷恢复该 child 并投递一次（`delivery-42`），`agent/created` 触发组合与证明，日志无任何 `surface-not-attested`/`locus composition refused`，投递 `replied`，child 工具面 16 个且 `subagent`/`subagent_fork` 均不在列——整链（读取 → 证明 → 发布）成立。）
- [x] 3.7 按生产分层重写 own 层负向单测（3.4 的剩余部分）：现有 `test/locus-safe-runtime.test.ts` 把被禁工具注册在 **global 层**，而真实泄漏发生在 **own 层**，因此该用例在任何情况下都是绿的。已实测确认平台约束：own 层工具**能穿过** `restrict(LOCUS_SAFE_TOOL_FILTER)` 且可执行（`core/tools/src/index.ts:1167-1174`「outside the filter above」），所以该用例**不能**断言 schemas 里没有它。改为两层断言：①特征化——按真实分层构造（standing 层放 `bash`/`subagent_fork`，`createScope(childKey, {parent: standingKey})` 的 own 层放 `subagent`），断言 own 层工具确实存活且可执行，把平台限制钉住，防止后人误以为 filter 已足够；②保证——Pet 侧组合证明必须把该 scope 判为泄漏，且发布路径据此拒绝（见 3.6）。（2026-09-19 已实现：`locus-safe-runtime.test.ts` 的分层特征化用例（断言 standing 层的 `bash`/`subagent_fork` 被滤除、own 层 `subagent` 存活且**可执行**、并断言证明把该 scope 判为 `leaked: ['subagent']`）；`locus-composition.test.ts` 新增证明三态与三条发布拒绝用例；`loader-composition.test.ts` 的 Host harness 补齐 `agents`/`tools` 读取以覆盖真实边界。所有新增断言已实测「实现回退即失败」。）
