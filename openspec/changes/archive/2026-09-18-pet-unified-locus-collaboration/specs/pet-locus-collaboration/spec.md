> **本 delta 已于 2026-09-15 合入 `openspec/specs/pet-locus-collaboration/spec.md`，不要再次应用。**
>
> 依据 `docs/adr/ADR-0004-consolidate-locus-specs-into-pet-locus-collaboration.md`：本文件的 13 条需求作为底稿写入主 spec，其中 3 条（全部飞书工作统一为子会话常规交互、上下文按实际子会话绑定并允许按需问主会话、投递与结算关联独立于会话运行状态）已被 `pet-locus-independent-child` 经真实飞书验收的新语义覆盖。主 spec 是这些需求的唯一真相源，本文件仅作历史留存。
>
> 本 change 的其余四个 capability（`dsh-runtime-provisioning`、`pet-qa-group`、`dsh-pet`、`pet-lark-channel`）**尚未处置**，其中 `pet-qa-group` 声明 REMOVED 全部 10 条。归档本 change 前必须逐个判断，否则会丢失这些变更。
