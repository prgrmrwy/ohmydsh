...
- [ ] 8.4 **部分完成，剩余项转入 BACKLOG B038**。已覆盖：scheduler 到期与 normal completion 的并发不会重复推进——`locus-expiry-scheduler.test.ts` 的 `does not advance the queue when the expiry CAS loses to a concurrent finish` 证明 expiry CAS 输给并发 finish 时不再调用 `finishAndAdvance`（该用例经变异验证：移除守卫后确实失败），补上了此前完全缺失的定时器侧证明。**仍未覆盖**：重启后不重复投递 current 的专项回归——`claimCurrentDeliveryMutation` 的 occupancy 检查在逻辑上防止了它，但没有「重启 → 再次 dispatch」的端到端断言；此项属于崩溃/重启窗口，与 8.3 同类，一并由 B038 承接
...
- [x] 10.6b 以单测替代真实验收（所有者确认）：`pet_locus_wait` 的效果在飞书界面不可见（只能查 durable `deadlineAt`/`revision`），且难以稳定构造 child 主动调用 wait 的真实场景。pure 层 `uses the fixed acceptance deadline and hard 24-hour cap for repeated waits`（反复 wait 被钉在 `acceptedAt+24h`）与 `treats a wait already covered by the live lease as satisfied, not as an expiry`，durable 层 `answers a wait already covered by the live lease with the intact deadline`；后两个均经反向验证（撤掉修复后确实失败）
- [x] 10.6c 以单测替代真实验收（所有者确认）：默认租约 1 小时且**代码中无任何可调短的环境变量或配置项**（`DEFAULT_DELIVERY_LEASE_MS`/`MAX_DELIVERY_LEASE_MS` 均为写死常量，`src/` 内唯一使用 `process.env` 之处是 `paths.ts` 的路径解析），不为验收临时改代码。**核查时发现真实测试空白**：到期 CAS 与 finish 后推进各有测试，但连接两者的定时器本身（原 `index.ts` 内联的 `scheduleCurrentDelivery`）位于 Host 闭包内，`grep` 确认无任何测试触碰——定时器装错延迟、`wait` 后漏 re-arm、CAS 失败仍推进队列，都不会被发现。已将其原样抽取为 `host/locus/expiry-scheduler.ts`（注入时钟与定时器，逻辑不变，`index.ts` 改为调用该模块），并新增 `locus-expiry-scheduler.test.ts` 9 例覆盖：按租约而非硬上限装定时器、到期后以重读的 revision 做 CAS 并推进、`deadlineAt` 损坏时不越过硬上限、`wait` 延长后 re-arm 而不过期、行已非 current 时静默退出、CAS 输给并发 finish 时不重复推进、重复 schedule 只保留一个定时器、已过期时延迟钳为 0、dispose 清空全部定时器。三次变异测试确认覆盖有效（移除 re-arm 守卫、去掉硬上限规整、CAS 失败仍推进，各自被对应用例精确捕获）
- [x] 10.7 已回填 `docs/notes/pet-locus-independent-child-handoff.md`（新增「2026-03-24 第二轮范围收敛」与「真实飞书验收（2026-09-15）」两节，含三个缺陷的完整背景、验证证据与方法论教训）与 `BACKLOG.md`（B035 状态行更新；B036 按所有者新语义扩写为「拉 bot 零副作用」；新增 B038 承接本 change 未完成的 8.3）。**归档顺序**：`pet-locus-independent-agent-inquiries`（B035 完整协作平台）仅完成 2/57 且为有意暂停，按 handoff 记录的「恢复 B035 的前置条件」不得先于本 change 归档，其归档待独立 child 与回复出口真实验收后另行判断——该前置条件现已由本 change 的 10.4/10.5/10.6a 真实验收满足；`pet-unified-locus-collaboration` 完成 58/59，是本 change 的基线 change，其归档时机由所有者另行决定，本 change 不代为处置
