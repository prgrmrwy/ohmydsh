...
**Reason**: 实机反馈三点问题——点击前不可感知标题可点；复制内容不可见于界面；面包屑是导航控件，标题点击复制语义违直觉。改为标题旁自建 6 位短标识徽标，点击徽标复制完整 id。
**Migration**: 复制入口从「点击标题」迁移为「点击标题右侧徽标」；标题恢复官方 disabled 原样。

### Requirement: 当前会话标题悬停显示 pointer 指针
**Reason**: 标题不再承担点击复制，恢复官方 cursor: default；pointer 指针与悬停底色迁移到徽标（见 ADDED「徽标 hover 提示与复制后轻量反馈」）。
**Migration**: 无用户动作迁移；标题自动恢复官方态。

### Requirement: 标题点击复制给出轻量即时反馈
**Reason**: 反馈主体从标题点击改为徽标点击（见 ADDED「徽标 hover 提示与复制后轻量反馈」）。
**Migration**: toast 语义与实现保留，触发源为徽标点击。
