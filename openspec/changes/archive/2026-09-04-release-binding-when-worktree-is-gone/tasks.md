...
- [x] 5.4 端到端验证写入修复：本机真实收尾已产出 tombstone 直接为 `cleaned-archived` 的记录，证明该路径不再写出卡死的裸 `cleaned`。实机证据（本仓库 `.git/ws/operations/`，非测试 fixture——包测试一律在 tmpdir 落盘）：`dce9f71e`（worktree `.worktrees/say-hi`）与 `e3ce9ec3`（`.worktrees/sayhi-1`）均为 `phase: cleaned` + `binding.state: cleaned-archived`，写入时间 2026-09-04T14:44:41Z / 14:47:47Z，晚于修复提交 `4850d26`（2026-09-04T12:45Z）且部署副本已含 `releaseMissingWorktreeBinding`/`worktreeGone`。修复前 `wsClean` 硬编码 `state:'cleaned'`，不可能产生该值。
- [x] 5.5 端到端验证归属判定：实机记录显示归属判定确实只跟随「托管 worktree 是否仍存在」。证据（同上目录）：`0e0bd1f2`（`.worktrees/task-6aa8f49cc9`，目录已删除）为 `phase: cleaned` + `state: released`，即已释放为普通会话；`496da281`/`b7bfb1f7`/`fe738106` 同为已删除 worktree 且 `released`；反向对照 `5c0619fb`（`.worktrees/zhangyong-617-prgrmr-163-com`，目录仍存在）保持 `phase: prepared` + `state: admitted`，未被误降级。四条记录的 worktree 存在性与绑定状态逐条吻合。
      注：以上为落盘状态的实机核验。任务原文另提到的「打开会话确认 `bash` 可用」属 UI 层瞬时观察，不落盘、无法从记录反向证明，故不在此声明；该行为的等价断言由自动化测试承载（`archive-then-clean-release.test.ts` 断言 cleaned 期间四类工具全拒、释放后回到 `released`；`release-missing-worktree.test.ts` 覆盖释放、不误降级、同名重建目录仍判 gone、未清理绑定拒绝释放）。`src/index.ts` 的 `agent/session-start` 编排时序（同步跳过 guard + 异步落盘）无独立自动化覆盖，已记入阶段四 DSH 0.1.2 迁移的回归基线。
