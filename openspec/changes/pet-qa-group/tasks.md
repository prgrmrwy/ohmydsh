...
- [x] 9.2 **重复点击 Q&A 会不断新建答疑群**。真机上同一源会话累积出三个同名群。
      ...
      建群的**产物**——以它为键的复用查找结构上永远不可能命中，复用判断发生在群
      已经建好之后，为时已晚。这是把二期1 的入站模型（消息先到，chatId 是已知
      输入）直接套到出站方向的疏漏：Q&A 点击时唯一已知的是源会话。
      修法：新增 `qa:<sourceSessionId>` 作用域键（与普通 `session:` 独立，使一个
      会话可同时持有浮层 Task 与答疑群），建群前先查活跃 qa Task——命中且绑定有效
      即返回既有群并标记 `reused`，不 fork、不建群、不写绑定；命中但绑定失效/缺失
      则归档该 Task 再新建（不返回无法应答的群）。归档 qa Task 因此成为「结束答疑群」
      的正式手段，也是重新建群的前置。客户端区分「已创建」与「已存在」两种提示，
      不再让两种结果外观相同。
      新增 4 个回归测试（复用同一群、不同会话各自建群、归档后可重建、失效绑定被
      替换）；spec 增补复用 Requirement 与三个 Scenario。
      存量：真机已有的三个同名群按用户决定不做迁移，由用户手工归档不需要的。

- [x] 9.3 **答疑群的群主是 bot 而非发起者**。`im +chat-create --as bot` 在不传
      `--owner` 时把群主默认为 bot（帮助文本明写 "defaults to bot"），而建群调用
      一直没传该参数。群主不是称谓而是权限：只有群主能改群名、拉人踢人、解散群、
      转让群——现状等于用户要请一个 agent 代为管理自己的群。
      修法：`createChat` 增 `ownerOpenId` 参数并透传 `--owner`；建群时把
      allowlist 首位（机器所有者）同时作为唯一受邀者与群主。spec 的建群
      Requirement 与 Scenario 均补上「群主是本人」。已有测试断言升级为三参数形式。
      存量：真机已建的三个群群主仍是 bot，按 9.2 的处理口径不迁移；用户可自行
      解散或请 bot 转让。

- [x] 9.4 **复用提示常驻且遮挡内容**（9.2 的呈现缺陷，真机截图发现）。9.2 新增的
      成功回执复用了 `dshpet-wheel-note`——那是给「Pet 未就绪」这类需要打断的短提示
      设计的 `position:absolute` 浮层，且只在下次点击时清除。于是一条本该读完即走的
      回执常驻在轮盘下方，压住其后的内容。
      修法：回执与错误分开对待——错误等待下一次尝试，回执读完即走。新增 6s 自动
      消失（替换时重置计时，不继承上一条的余量）与 `dshpet-wheel-receipt` 修饰类：
