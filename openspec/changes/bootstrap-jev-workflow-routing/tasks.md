...
- [ ] 6.3 已完成 16-case 人工标注中英文 synthetic fixture，并实现/验证只输出 aggregate 的版本化 report（完整混淆矩阵、加权成本、needs-review/失败率、p50/p95、usage、覆盖告警；无定价时成本明确 unavailable）；live Jev 接通后已有 1 条 prospective existing-change 样本并可靠标注为 standard-openspec，仍远低于 100 labels / 10 working days / 各 route 与高代价类别覆盖门槛；禁止读取历史 prompt 回填，继续随真实 vibe 使用积累
