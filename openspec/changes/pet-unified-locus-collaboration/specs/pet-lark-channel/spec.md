...
### Requirement: 回复授权按消息来源判定而非按是否为 Delivery 一刀切

判定一轮是否具备回复授权时，系统 SHALL 按 inbox 消息的来源区分：宿主自注入的上下文（工作区指令、运行时快照、skill 目录）MUST NOT 被视为混入的外部流量，也 MUST NOT 因其存在而撤销该轮回复授权。参与者流量中无法对应到 Delivery 的消息（如 GUI 提问、父会话 steer）SHALL 继续使该轮失去回复授权。来源不可知时 SHALL 按参与者流量处理，不得因读不到来源而放行。

系统 SHALL 区分「暂未解析到 Delivery」与「证实非 Delivery」两种状态：入队唤醒与持久绑定之间存在固有窗口，claim 先于持久绑定到达 MUST NOT 触发不可逆的授权撤销。未决期间 SHALL 拒绝发送；解析证实该轮恰有一条 Delivery 且无已证实污染后，SHALL 恢复该轮回复授权。永久撤销 SHALL 仅由已证实的事实触发：同轮第二条 Delivery、解析基础设施失败、或证实为外部流量。

#### Scenario: 首轮携带宿主注入上下文
- **WHEN** 子会话首次收到飞书消息，该轮同时包含宿主自注入的工作区指令、运行时快照与 skill 目录
- **THEN** 该轮仍具备回复授权，子会话可向原入口发送业务正文

#### Scenario: claim 先于持久绑定到达
- **WHEN** 子会话轮次开始时其 Delivery 尚未完成持久绑定，绑定随后完成并证实该轮恰有这一条 Delivery
- **THEN** 该轮恢复回复授权，子会话可发送业务正文；未决期间不发送

#### Scenario: 真实外部流量仍然撤销授权
- **WHEN** 同一轮中混入无法对应到 Delivery 的参与者消息
- **THEN** 该轮失去回复授权，不向任何入口发送业务正文，且不因后续解析恢复
