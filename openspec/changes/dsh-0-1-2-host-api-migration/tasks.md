...
- [x] 5.3 验证 sidebar-qa 划选提问实际可用,以及依赖方 `session-links` 的「文档/资料」tab 仍正常注册与计数。
...
- [x] 6.1 隔离环境验收通过后,回主 checkout 物化并复跑基线;确认日常 GUI 全部插件正常加载(启动清单项数与预期一致)。
  - ✅ 阻塞已解除(2026-09-05):cockpit 双栈适配已归档、bridge 0.3.0 已发布并跟进。**执行顺序更正**:必须先合并任务分支再物化(~/.dsh 装的是主 checkout 源码)。
