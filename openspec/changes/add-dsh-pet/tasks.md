...
- [ ] 13.1 Replace the rectangular hover menu with a concentric-ring wheel.
  Geometry (mascot 72px reference): 20px breathing gap, 38px ring width, ring
  capacities 6/8/10 for a hard ceiling of 24. Text runs tangential to the arc
  and flips 180° whenever that angle would render upside-down, so the bottom
  sector reads fully upright. Labels truncate to the arc's capacity (about
  5/6/7 characters per ring); a tooltip appears only when the capability has a
  description, formatted `name: description`.

  The interactive disc must match what is DRAWN, not the maximum three rings:
  measuring against a fixed three-ring radius kept the wheel open over blank
  space where later rings would be. Ring 1 appears immediately, each further
  ring 0.08s later.

  Only the mascot opens the wheel — binding hover to the 344px container made
  the empty area around a collapsed mascot a hover target. Closing needs no
  grace period because the disc is continuous from the centre outward: the
  breathing gap and the seams between segments are all inside the radius, so
  leaving it means the pointer genuinely left.

  Mockup: `design-notes/wheel-mockup.html`.

- [ ] 13.2 Drop `petConfirm` end to end: frontmatter field, parser, the
  client's two-click flow, and the example Skill declarations.

  A capability now runs on a single click. Safety belongs to the Skill inside
  its Pet Task — a blanket confirmation at the entry point cannot tell a
  destructive capability from a harmless one, so it taxes every action without
  actually protecting the dangerous ones.

- [ ] 13.3 Simplify the Task panel to one clickable row per Task, opening the
  executor session. Remove `Open source`, `Archive` and `Cancel`.

  Archiving stays available where it belongs — in the session itself — and
  `reconcileArchives` already observes that live: a terminal Task archives
  automatically, while a non-terminal one stays active with a diagnostic
  rather than treating an external archive as proof the work was cancelled.

- [ ] 13.4 Cap enabled Skills at 24 in Settings, and ignore any beyond that at
  render time so an over-capacity state degrades instead of failing.
