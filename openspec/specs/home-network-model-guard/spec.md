...
当客户端 composer block 生效时，输入区 SHALL 保持可见且保有至少一行文本的高度，其阻断原因文案 SHALL 可被用户读到；草稿为空时同样如此。系统 MUST NOT 让被阻断的输入区塌陷到无法辨识原因的程度——阻断的可感知性是该 affordance 的组成部分，仅把原因写入 DOM 而用户看不见（例如因容器高度归零被溢出裁切）不满足本要求。

阻断 SHALL 只禁用发送，MUST NOT 丢弃或隐藏用户已输入的草稿内容。
...
#### Scenario: 被阻断且草稿为空时输入区保持可读

- **WHEN** 出口被判定为阻断或未知，客户端对当前会话写入 composer block，且该会话草稿为空
- **THEN** 输入区仍占据至少一行文本的高度，阻断原因文案可被用户读到，而非塌陷为不可辨识的细条

#### Scenario: 被阻断时已有草稿保持不变

- **WHEN** 出口被判定为阻断或未知，而该会话输入框已有草稿内容
- **THEN** 草稿内容保持不变且依然可见，输入区高度不受影响，仅发送被禁用
