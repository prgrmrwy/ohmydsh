# pet-compat-minimization Specification

## Purpose

约束 Pet 对宿主源码 compatibility patch 的使用：任一 seam 的**新增、保留与移除**都必须以对目标版本官方已发布 API 的正面举证为前提，并使 compat 面保持在可逐项辩护的最小规模。

本 capability 的存在源于一次实测：Pet 的 compat overlay 一度替换 7 个上游包 / 72 hunks，而逐条复核后其中多数并不成立——它们来自"当时没查官方 API"或"查了一半就下结论"，而非官方确实无法表达。
