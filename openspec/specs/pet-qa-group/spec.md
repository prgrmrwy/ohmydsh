# Pet QA Group Specification

## Purpose

答疑群：把一个已经建立了上下文的 DSH 会话开放给他人提问。点击 Pet 轮盘的 Q&A
内置动作后，Host 将该会话 fork 成 continuable 子代理、创建仅含本人与 bot 的飞书
群并绑定二者；本人再拉人进群，任何群成员 @bot 的提问都由这个"记得那段工作"的
child 回答，而不是一个从零读代码的新 executor。

本能力覆盖：建群事务与回滚、`kind: qa` 绑定与群成员准入豁免、消息经宿主队列投递
child、parent 唤活与 child 冷恢复、子代理结算驱动的表情反馈、源会话失效的 fail
closed 语义、child 的工作目录约束，以及 GUI 收纳与私聊通道的边界。
