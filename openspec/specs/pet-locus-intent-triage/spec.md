...
子会话 SHALL 对每条 current Delivery 判定其属于**信息交换**（同步、问询、解释、定位）还是**要求干活**（修改代码、修复缺陷），并据此选择当场答复或登记待办。判定 SHALL 由子会话自身做出；Host MUST NOT 对消息内容做分类，也 MUST NOT 依据关键词自动改写判定结果。

权限边界 MUST NOT 依赖该判定：locus 仍默认只读，判定为"要求干活"MUST NOT 使子会话获得任何写能力，判定为"信息交换"也 MUST NOT 使其失去既有只读能力。误判的唯一后果是结局选择不当，MUST NOT 导致越权。

两类结局都 SHALL 经既有 `pet_locus_finish` 结算该 Delivery；判定本身 MUST NOT 成为第二个完成入口。
...
子会话无法判定意图时 SHALL 向当前飞书入口澄清一次，说明需要确认的是"要答案"还是"要改动"。澄清 SHALL 使用当前 Delivery 的既有回复路径，后续 at 回答在同一子会话继续。

同一条 current Delivery SHALL 至多澄清一次。已澄清过仍无法判定时，或澄清未获可判定答复时，系统 SHALL 登记待办，MUST NOT 当作信息交换处理——产生一条可由所有者关闭的待办，优于让请求消失。

澄清 MUST NOT 消耗或结算 current Delivery，也 MUST NOT 绕过既有 deadline 与超时推进规则。

#### Scenario: 澄清后得到可判定答复
- **WHEN** 子会话就意图澄清，用户随后 at 回答"只要你看一下原因"
- **THEN** 后续轮次在同一子会话继续，按信息交换处理

#### Scenario: 澄清未获答复仍登记
- **WHEN** 子会话已澄清一次，但未得到可判定答复
- **THEN** 系统登记待办并结算该 Delivery，MUST NOT 因无法判定而按信息交换静默放过

#### Scenario: 不重复澄清
- **WHEN** 同一条 current Delivery 已澄清过一次，子会话仍不确定
- **THEN** 系统不再发出第二次澄清，直接登记待办
