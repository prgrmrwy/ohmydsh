...
- **更新**: 2026-09-13 在 T8 后由所有者发现并指出「是不是不应该 fork……要不现在这样看起来还是没有 locus 同构的那种效果」；证据链完整保留于本条。
...
- **更新**: 2026-09-13 在 T4 验收创建默认 Q&A 成功后发现（截图为证）。
...
- **更新**: 2026-09-13 在 T4 验收中由所有者指出；创建路径以轮盘为唯一入口，设置页改为只读展示。
...
### [B029] Bot 被移出群时标记入口不可达（不自动解绑）
- **状态**: 已设计
- **优先级**: P1
- **背景 / 动机**: 当前只订阅了 `im.chat.member.bot.added_v1`，没有对应的移除事件。把 bot 移出群后，该群的 locus 仍停留在 `active`、管理面继续展示为有效关联，但消息实际已无法送达——这是**伪报可用**，与刚修复的「伪报不可用」（10.4）属同一类缺陷、方向相反。入群能自动建立关联，退群完全无感知，生命周期不对称。
...
- **要点**:
  - 订阅 bot 被移出群的事件（飞书侧对应 `im.chat.member.bot.deleted_v1`）。实际事件字段与投递行为**必须用真实事件实测**，不能照文档推断，见 `docs/notes/dsh-plugin-integration-pitfalls.md` 第 4 节。
  - 收到事件后：保留 locus 记录与全部历史，**仅标记「入口不可达」并记录时间**；管理面明示「bot 已不在该群，消息无法送达」，并就近提供解绑/归档入口，由所有者决定是否收敛。
