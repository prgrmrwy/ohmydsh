...
- **部署现状(2026-08-24 最终)**: 终选 **SSH 隧道**为日常形态。`lan-gate` `enabled: false`(条目保留可回退),`web.lan` 保持 false,DSH 绑回 `127.0.0.1`,局域网零端口暴露;未安装任何代理插件。中途曾短暂启用直连形态并实测生效(启动行打印 `dsh web: http://127.0.0.1:3080 (LAN: http://192.168.64.3:3080)`,绑定 `*:3080`),确认可用后按用户意愿回退,残留策略文件 `~/.dsh/lan-gate.json` 已清理。隧道用法见 `docs/notes/lan-access-ssh-tunnel.md`。
