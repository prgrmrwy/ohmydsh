...
- **更新**: 2026-09-11 在统一 Locus 人工验收中记录；验收完成后统一评估和实施，当前不改主线。
...
- **更新**: 2026-09-12 在 T3 验收中实测记录（话题 A 两条投递 58 行中 55 行重复）。

### [B029] Bot 被移出群时自动解绑对应 locus
- **状态**: 想法
- **优先级**: P1
- **背景 / 动机**: 当前只订阅了 `im.chat.member.bot.added_v1`，没有对应的移除事件。把 bot 移出群后，该群的 locus 仍停留在 `active`，管理面继续展示为有效关联，所有者必须手动解绑/归档才能清理。入群能自动建立关联，退群却不能自动收敛，生命周期是不对称的。
- **要点**:
  - 订阅 bot 被移出群的事件（飞书侧对应 `im.chat.member.bot.deleted_v1`，实际字段与投递行为必须先用真实事件实测，不能照文档推断——见 `docs/notes/dsh-plugin-integration-pitfalls.md` 第 4 节）。
  - 明确「移出群」应落到哪个终态：停止（保留历史、可显式重建）还是归档，并与现有 `/unbind`、停止标记语义一致；群级退出不得级联改变仍有效的话题 locus。
  - 幂等与乱序：重复投递、移除后重新拉入、事件早于/晚于消息到达都必须收敛到同一结果；重新入群应走显式重建而非静默复活（沿用现有停止标记规则）。
  - 忙时处理：有执行中或排队的 Delivery 时如何收敛（拒绝、等待还是标记待清理），不得丢失在途工作或错配结算。
  - 权限边界：事件只证明 bot 不在群里，不证明操作者身份；不得据此推断所有者意图做更大范围的清理。
- **更新**: 2026-09-12 在真实群验收中发现；当前仅记录，不改验收主线。
...
- **更新**: 2026-09-12 在真实群 T3 验收中记录：3 条 locus（群级 gen1 已停止、群级 gen2 活跃、话题 A 活跃）已导致「数量不对」的误判；建议与 B026 合并设计。
