...
function usage() {
  return `Usage: node scripts/fetch-rc2-connection-source.mjs [options]\n\n` +
    `Options:\n` +
...
function defaultCacheDir() {
  ...
  return path.join(root, 'ohmydsh', 'dsh-federation', 'connection-source')
}
...
async function validateArchive(archivePath, manifest) {
  ...
  if (digest !== manifest.archive.sha256 || bytes.byteLength !== manifest.archive.size) {
    throw new Error(`connection source archive mismatch: expected ${manifest.archive.sha256}/${manifest.archive.size}, got ${digest}/${bytes.byteLength}`)
  }
}
...
async function validateExtracted(root, manifest) {
  for (const entry of manifest.blobs) {
    ...
    if (bytes.byteLength !== entry.size || gitBlob(bytes) !== entry.gitBlob) {
      throw new Error(`connection source blob mismatch for ${entry.path}`)
    }
  }
}

async function bootstrap(cachePath, manifest) {
  const work = await mkdtemp(path.join(tmpdir(), 'ohmydsh-connection-source-'))
...
export async function fetchConnectionSource(options = {}) {
  ...
  try {
    ...
    if (options.offline) throw new Error(`offline connection source cache miss or corruption at ${cachePath}`, { cause: error })
...
async function main() {
  ...
  const result = await fetchConnectionSource(options)
  process.stdout.write(options.json ? `${JSON.stringify(result)}\n` : `[connection-source] ${result.source}: ${result.archiveSha256}\n`)
}

if (process.argv[1] && path.resolve(process.argv[1]) === fileURLToPath(import.meta.url)) {
  main().catch(error => {
    console.error(`[connection-source] ERROR ${error.message}`)
    process.exitCode = 1
  })
}
