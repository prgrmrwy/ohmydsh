 ...
 * top-level DSH_LAN switch and the per-customization `enabledEnv` field, so
 * both mechanisms carry the same meaning for the same spellings.
 */
 ...
 * truthy/falsy spelling (case-insensitive) returns true/false. Shared by
 * DSH_LAN (resolveWebLan) and per-customization `enabledEnv` (loadManifest).
 */
...
function resolveWebLan(manifest) {
  const override = resolveEnabledOverride(process.env.DSH_LAN)
  return override !== undefined ? override : manifest.web.lan
}
...
function loadManifest() {
  ...
  const items = doc.customizations.map((item, index) => {
    ...
    // machine-specific opt-in without touching dsh.yaml (mirrors web.lan /
    // DSH_LAN, scoped to one customization instead of the whole profile).
