...
describe('caller-bound shared fact updates', () => {
  it('records the writing child locus and generation, derived by the Host', async () => {
    const f = fixture()
    const record = await updateCollaborationContextForCaller({ agent: { id: 'child' } }, replacement(), f)
  ...
  it('records a main-session writer as the parent, not as a child locus', async () => {
    const f = fixture()
    const record = await updateCollaborationContextForCaller({ agent: { id: 'main' } }, replacement(), f)
  ...
  it('ignores any model-supplied parent, author, time or scope fields', async () => {
    const f = fixture()
    await expect(updateCollaborationContextForCaller({ agent: { id: 'child' } }, {
  ...
  it('propagates a revision conflict so the caller rereads instead of overwriting', async () => {
    const f = fixture()
    await updateCollaborationContextForCaller({ agent: { id: 'child' } }, replacement(), f)
    await expect(updateCollaborationContextForCaller({ agent: { id: 'main' } }, replacement(0), f))
      .rejects.toMatchObject({ code: 'REVISION_CONFLICT' })
    expect((f.current() as { revision: number }).revision).toBe(1)
  })

  it.each([undefined, {}, { agent: { id: 'stranger' } }])('denies unproven caller %j before writing', async execution => {
    const f = fixture()
  ...
  it('denies a retired child and leaves the current revision untouched', async () => {
    const f = fixture()
    f.retire()
    await expect(updateCollaborationContextForCaller({ agent: { id: 'child' } }, replacement(), f)).rejects.toMatchObject(denied)
    expect(f.update).not.toHaveBeenCalled()
    expect((f.current() as { revision: number }).revision).toBe(0)
  })
  ...
  it('does not expose storage diagnostics when the atomic write fails', async () => {
    ...
    const failure = await updateCollaborationContextForCaller({ agent: { id: 'child' } }, replacement(), f).catch((error: Error) => error)
