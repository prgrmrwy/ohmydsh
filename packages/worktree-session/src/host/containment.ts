...
function doesNotEscape(ancestor: string, candidate: string): boolean {
 ...
 */
export async function confinePath(base: string, entry: string | undefined, options: { requireAbsolute?: boolean } = {}): Promise<ContainmentResult> {
  const lexRoot = normalize(base)
  const physRoot = normalize(await realpath(base))
  if (entry === undefined || entry === '') return { allowed: false, reason: `worktree root policy requires an explicit managed-root path: ${physRoot}` }
  const absolute = isAbsolute(entry) ? entry : (options.requireAbsolute === true ? '' : resolve(lexRoot, entry))
  if (absolute === '') return { allowed: false, reason: 'worktree root policy rejects relative paths; provide an absolute managed-root path' }
  ...
  if (!doesNotEscape(lexRoot, normalized)) return { allowed: false, reason: `path ${normalized} escapes the managed root ${normalize(base)}` }
  ...
  if (resolved !== null && !doesNotEscape(physRoot, resolved)) return { allowed: false, reason: `resolved path ${resolved} escapes the managed root ${physRoot}` }
 ...
 */
export async function resolveRealInside(baseRoot: string, path: string): Promise<string | null> {
  try {
...
export function confineWorkdir(base: string, workdir: string | undefined): ContainmentResult {
  const root = normalize(base)
  if (workdir === undefined || workdir === '') return { allowed: false, reason: 'bash requires an explicit workdir inside the managed root' }
  if (!isAbsolute(workdir)) return { allowed: false, reason: 'bash workdir must be an absolute managed-root path' }
  const normalized = normalize(workdir)
  if (!doesNotEscape(root, normalized)) return { allowed: false, reason: `bash workdir ${normalized} escapes the managed root ${root}` }
  return { allowed: true, path: normalized }
}

/** Unwrap a wrapper object argument (e.g. { filePath }) into a raw path string. */
export function firstPathOf(args: unknown): string | undefined {
