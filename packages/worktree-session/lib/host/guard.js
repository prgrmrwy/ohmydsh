...
function lexicalWithin(root, candidate) {
    const rel = relative(root, candidate);
    return rel === '' || (!rel.startsWith('..' + sep) && rel !== '..' && !isAbsolute(rel));
}
function firstPathObject(args) {
 ...
 */
export function checkTool({ name, args }, operation, validationFailure, continuableDelegationTools = []) {
    const binding = bindingOf(operation);
    if (binding === undefined || binding.mode !== 'source-session')
        return undefined;
    if (binding.state === 'cleaned')
        return `Worktree Session ${operation.operationId} 已清理；禁止继续操作旧执行目录`;
    const contract = TOOL_CONTRACTS[name];
    if (validationFailure !== undefined && (contract === undefined || contract.kind !== 'non-local'))
        return `Worktree Session 绑定校验失败，已禁止本地工具：${validationFailure}`;
    if (contract === undefined) {
        ...
        if (name.startsWith('subagent') || name === 'ralph' || name === 'workflow')
            return `Worktree Session 无法证明委派工具 ${name} 在首步前继承托管执行目录，已拒绝调用`;
    ...
    if (contract.kind === 'delegation') {
        if (name === 'send_message')
            return undefined;
        if (!continuableDelegationTools.includes(name) || firstPathObject(args).run_in_background === false)
            return `Worktree Session 只允许 Host 配置已审计且可证明首步前继承策略的 continuable background delegation：${name}`;
        return undefined;
    }
    ...
    if (contract.kind === 'bash') {
        ...
        if (workdir === undefined || workdir === '')
            return `bash 需要显式指定托管执行目录（worktree）内的 workdir：${managedRoot}`;
        if (!isAbsolute(workdir))
            return 'bash workdir 必须是托管执行目录内的绝对路径';
        const normalized = normalize(workdir);
        if (!lexicalWithin(managedRoot, normalized))
            return `bash workdir ${normalized} 超出托管执行目录 ${managedRoot}`;
        return undefined;
    }
    if (contract.kind === 'search') {
        const path = firstPathOf(args);
        if (path === undefined)
            return 'worktree root policy 要求显式指定托管执行目录内的搜索根';
        if (!isAbsolute(path))
            return `搜索工具必须使用托管执行目录内的绝对路径：${path}`;
        if (!lexicalWithin(managedRoot, normalize(path)))
            return `搜索根 ${path} 超出托管执行目录 ${managedRoot}`;
        return undefined;
    }
    ...
    for (const key of contract.pathFields) {
        ...
        const entries = Array.isArray(raw) ? raw.filter((v) => typeof v === 'string') : (typeof raw === 'string' ? [raw] : []);
        for (const entry of entries) {
            if (!isAbsolute(entry))
                return `文件工具必须使用托管执行目录内的绝对路径：${entry}`;
            if (!lexicalWithin(managedRoot, normalize(entry)))
                return `文件目标 ${entry} 超出托管执行目录 ${managedRoot}`;
        }
    }
    return undefined;
}
function auditedPaths(name, args) {
    const contract = TOOL_CONTRACTS[name];
    if (contract === undefined || contract.kind === 'delegation' || contract.kind === 'maintenance' || contract.kind === 'non-local')
        return [];
    const object = firstPathObject(args);
...
export async function physicalDecision(exec, operation) {
    if (bindingOf(operation)?.state === 'cleaned')
        return { kind: 'allow' }; // monotonic guard owns the stable terminal denial
    const contract = TOOL_CONTRACTS[exec.name];
    if (contract === undefined || contract.kind === 'delegation' || contract.kind === 'non-local')
        return { kind: 'allow' };
...
export function installGuard(agent, operation, validationFailure, continuableDelegationTools = []) {
    const disposeGuard = agent.ctx.tools.guard((exec) => checkTool({ name: exec.name, args: exec.arguments }, operation, validationFailure, continuableDelegationTools));
