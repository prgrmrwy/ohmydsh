...
export function activeBindingContext(operation) {
    return [
...
export function cleanedBindingContext(operation) {
    return [
...
export function boundContextText(operation, binding) {
    if (operation === undefined)
        return undefined;
    if (binding?.mode === 'source-session' && binding.state === 'cleaned')
        return cleanedBindingContext(operation);
    if (binding?.mode === 'source-session')
        return activeBindingContext(operation);
    return undefined;
}
/**
 ...
 */
export function installContext(agent, operation) {
    if (agent === undefined)
        return;
