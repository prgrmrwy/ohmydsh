...
export function createGitClient(runner = runProcess) {
    return {
        runner,
...
export async function discoverRepo(path, git = createGitClient()) {
    if (!isAbsolute(path) || !(await isDirectory(path)))
        throw new WsError('NOT_A_REPOSITORY', 'Repository path must be an existing absolute directory');
    const top = await git.maybe(path, ['rev-parse', '--show-toplevel']);
    if (top === undefined)
        throw new WsError('NOT_A_REPOSITORY', `Not inside a Git repository: ${path}`);
...
export async function listRefs(repoRoot, git = createGitClient()) {
    ...
    for (const line of output.split('\n')) {
        ...
        if (fullName === undefined || shortName === undefined || commit === undefined)
            continue;
        if (fullName.startsWith('refs/remotes/') && shortName.endsWith('/HEAD'))
            continue;
...
export async function resolveCommit(repoRoot, ref, git = createGitClient()) {
    if (ref.trim() === '' || ref.startsWith('-'))
        throw new WsError('INVALID_REQUEST', 'Base ref must be non-empty and must not start with -');
    const result = await git.maybe(repoRoot, ['rev-parse', '--verify', `${ref}^{commit}`]);
    if (result === undefined || result.trim() === '')
        throw new WsError('INVALID_REQUEST', `Base ref cannot be resolved to a commit: ${ref}`);
    return result.trim();
}
export async function listWorktrees(repoRoot, git = createGitClient()) {
    ...
    for (const line of output.split('\n')) {
        ...
        else if (current !== undefined && line === 'bare')
            current.bare = true;
        else if (current !== undefined && line === 'detached')
            current.detached = true;
        else if (current !== undefined && line.startsWith('prunable'))
            current.prunable = true;
    }
    flush();
    return entries;
}
export async function worktreeStatus(path, git = createGitClient()) {
    return git.run(path, ['status', '--porcelain=v1', '--untracked-files=all']);
}
export function taskSlug(taskText) {
...
export async function validateBranch(branch, repoRoot, git = createGitClient()) {
    const result = await git.runner('git', ['check-ref-format', '--branch', branch], { cwd: repoRoot });
    if (result.code !== 0)
        throw new WsError('INVALID_REQUEST', `Generated branch is invalid: ${branch}`);
}
export async function branchExists(repoRoot, branch, git = createGitClient()) {
    return (await git.maybe(repoRoot, ['show-ref', '--verify', '--quiet', `refs/heads/${branch}`])) !== undefined;
}
export async function allocateTask(repoRoot, taskText, git = createGitClient()) {
...
export async function createTaskWorktree(repoRoot, branch, path, baseCommit, git = createGitClient()) {
    if (await branchExists(repoRoot, branch, git)) {
        const existingCommit = (await git.run(repoRoot, ['rev-parse', '--verify', `refs/heads/${branch}^{commit}`])).trim();
        if (existingCommit !== baseCommit)
            throw new WsError('OPERATION_CONFLICT', `Existing task branch ${branch} does not match the recorded base commit`);
...
export async function pruneInvalidRegistrations(repoRoot, git = createGitClient()) {
    const paths = [];
    for (const entry of await listWorktrees(repoRoot, git)) {
        if (!entry.prunable || await isDirectory(entry.path))
            continue;
        paths.push(entry.path);
    }
    if (paths.length > 0)
        await git.run(repoRoot, ['worktree', 'prune', '--expire', 'now']);
    return paths;
}
export function withinRepo(repoRoot, candidate) {
