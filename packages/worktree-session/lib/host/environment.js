...
export async function ensureWorktreeExclude(gitCommonDir) {
...
export function managedEnvironment(content, dshHome) {
...
async function ignoredByGit(repoRoot, relativePath, git) {
    const result = await git.runner('git', ['check-ignore', '--quiet', '--', relativePath], { cwd: repoRoot });
    return result.code === 0;
}
export async function prepareEnvironment(repoRoot, worktreePath, gitCommonDir, operationId, git = createGitClient()) {
    const source = join(repoRoot, '.env.local');
    const destination = join(worktreePath, '.env.local');
    if (await pathExists(source)) {
        if (!(await ignoredByGit(repoRoot, '.env.local', git)))
            throw new WsError('ENVIRONMENT_FAILED', 'Refusing to synchronize .env.local because the source is not Git-ignored');
