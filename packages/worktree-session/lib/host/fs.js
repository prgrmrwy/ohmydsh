...
export async function atomicWrite(path, contents, mode) {
...
export async function withMkdirLock(lockPath, action, options = {}) {
    ...
    for (;;) {
        try {
            ...
            if (Date.now() >= deadline)
                throw new WsError('OPERATION_CONFLICT', `Timed out waiting for lock ${lockPath}`, { retryable: true });
...
export async function touchExclusive(path) {
