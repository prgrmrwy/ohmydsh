...
import { allocateTask, branchExists, createGitClient, createTaskWorktree, discoverRepo, listWorktrees, pruneInvalidRegistrations, resolveCommit, withinRepo } from './git.js';
import { runProcess } from './process.js';
const flights = new Map();
export function operationFile(gitCommonDir, operationId) {
...
function validateOperationId(id) {
    if (!/^[A-Za-z0-9][A-Za-z0-9._-]{7,127}$/.test(id) || id.includes('..'))
        throw new WsError('INVALID_REQUEST', 'operationId must be 8-128 safe characters');
}
function hashTask(text) {
    return createHash('sha256').update(text).digest('hex');
}
function validateReplay(existing, request, repoRoot) {
...
function resultOf(operation) {
    if (operation.phase !== 'prepared' || operation.lockFingerprint === undefined)
        throw new WsError('OPERATION_INVALID', 'Operation is not prepared', { retryable: true, phase: operation.phase });
    return {
...
async function validateResource(operation, phase, git, runner) {
    if (phase === 'branch-created')
        return branchExists(operation.repoRoot, operation.taskBranch, git);
    if (phase === 'worktree-created') {
        if (!(await isDirectory(operation.worktreePath)))
            return false;
    ...
    if (phase === 'dependencies-ready') {
        if (operation.lockFingerprint === undefined || operation.cacheNodeModules === undefined)
            return false;
        const expected = await dependencyFingerprint(operation.worktreePath, runner);
        if (expected.fingerprint !== operation.lockFingerprint)
            return false;
    ...
    if (phase === 'environment-ready' || phase === 'prepared') {
        if (!(await validateResource(operation, 'worktree-created', git, runner)))
            return false;
        if (operation.dependencyMode === 'lean' && !(await validateResource(operation, 'dependencies-ready', git, runner)))
            return false;
        if (operation.dshHome === '' || !(await isDirectory(operation.dshHome)))
            return false;
...
async function readJsonOrText(path) {
    try {
...
async function performStart(request, deps) {
    validateOperationId(request.operationId);
    if (request.taskText.trim() === '')
        throw new WsError('INVALID_REQUEST', 'taskText must be non-empty');
    if (request.dependencyMode !== 'lean')
        throw new WsError('INVALID_REQUEST', 'Only lean dependency mode is supported at start');
    ...
    return withMkdirLock(lock, async () => {
        ...
        if (operation !== undefined)
            validateReplay(operation, request, repo.repoRoot);
        if (operation === undefined) {
        ...
        if (operation.phase === 'cleaned')
            throw new WsError('OPERATION_CONFLICT', 'A cleaned operation cannot be restarted');
        if (operation.phase === 'prepared' && await validateResource(operation, 'prepared', git, runner))
        ...
        try {
            ...
            if (!(await validateResource(operation, 'worktree-created', git, runner)))
                throw new WsError('OPERATION_INVALID', 'Created worktree failed validation', { phase: 'worktree-created', retryable: true });
...
export function startOperation(request, deps = {}) {
